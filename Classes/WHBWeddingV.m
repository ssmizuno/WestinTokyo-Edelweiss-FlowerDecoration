...
- (id)init{
	if((self = [super init])){
		...
		UIGradationView* bg = [[[UIGradationView alloc]initWithFrame:CGRectMake(26,11,450,657)]autorelease];
		...
		tabButton0 = [UIButton Tabpoint:CGPointMake(251-113,30) title:StrTabTitle0 imageName:ImageName_TabOff delegate:self tag:0 jp:NO];//186
		tabButton1 = [UIButton Tabpoint:CGPointMake(251+113,30) title:StrTabTitle1 imageName:ImageName_TabOff delegate:self tag:1 jp:NO];
		
		[self addSubview:tabButton0];
		[self addSubview:tabButton1];
		
		
		CGRect frameTable = CGRectMake(26,50,450,618);
...
- (void) buttonAction:(UIButton*)sender{
	...
	}else if(sender.tag == TagEveningButton){
		[self.controller moveTo:WHMenuBouquet2 option:nil];
	}
}
